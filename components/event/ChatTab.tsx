...
function MessageBubble({ msg, isMe }: { msg: ChatMessage; isMe: boolean }) {
  ...
  const name     = msg.sender?.displayName?.trim() || msg.sender?.username?.trim() || 'User';
...
const b = StyleSheet.create({
  row:         { flexDirection: 'row', gap: 8, paddingHorizontal: 14, marginBottom: 12 },
  ...
  orgText: { fontFamily: fontFamily.semibold, fontSize: 10, color: brand.primary },
  ...
  bubbleText:  { fontFamily: fontFamily.regular, fontSize: fontSize.sm, color: neutral[800], lineHeight: 20 },
...
export default function ChatTab({ eventId }: Props) {
  ...
  useEffect(() => {
    ...
    const join = () =>
      socket.emit('join:event-chat', { eventId, section: SECTION_KEY[section] });
        ...
        {SECTIONS.map((sec) => {
          ...
          return (
            <TouchableOpacity
              ...
              activeOpacity={0.8}
            >
              <Text style={[s.tabText, active && s.tabTextActive]}>{sec.label}</Text>
            </TouchableOpacity>
          );
        })}
      </View>
      ...
      <View style={s.inputBar}>
        {isAuthenticated ? (
          <>
            ...
            />
            <TouchableOpacity
              style={[s.sendBtn, (!input.trim() || !isConnected) && s.sendBtnDim]}
              onPress={handleSend}
...
const s = StyleSheet.create({
  ...
  tabs: {
    ...
    paddingTop: 10, paddingBottom: 4, gap: 6,
  },
  tab: {
  ...
  signIn: {
    ...
    fontFamily: fontFamily.regular, fontSize: fontSize.sm,
    color: neutral[400], paddingVertical: 8,
  },
});
