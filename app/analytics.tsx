...
const kpi = StyleSheet.create({
  ...
  label: { fontFamily: fontFamily.regular, fontSize: 10, color: neutral[500] },
  value: {
...
// 2-column KPI grid — each card takes exactly half width minus gap
function KPIGrid({ children }: { children: React.ReactNode }) {
  return <View style={kg.grid}>{children}</View>;
}

const HALF = "48%" as any;
  ...
  return (
    <View style={em.wrap}>
      <View style={em.iconWrap}>
        <Ionicons name={icon} size={24} color={neutral[300]} />
      </View>
...
function SummaryTab({ eventId }: { eventId: string }) {
  const { data, isLoading, isError, refetch } =
    useGetEventAnalyticsQuery(eventId);
  ...
  return (
    <View style={tab.wrap}>
      {/* KPI grid */}
      <KPIGrid>
        <View style={{ flexBasis: HALF, flexShrink: 0 }}>
          <KPICard
            label="Total RSVPs"
            value={fmt(rsvpTotal)}
        ...
        <View style={{ flexBasis: HALF, flexShrink: 0 }}>
          <KPICard
            label="Check-ins"
            value={fmt(checkinTotal)}
            iconName="checkmark-done-outline"
            color="#2563eb"
            sub={`${fmtPct(checkinRate)} rate`}
          />
        </View>
        <View style={{ flexBasis: HALF, flexShrink: 0 }}>
          <KPICard
            label="Tickets Sold"
            value={fmt(ticketsSold)}
      ...
      {rsvpTotal > 0 && (
        <SectionCard title="RSVP → Check-in Conversion">
          <View style={tab.convRow}>
            ...
            <Text style={tab.convSub}>
              {fmt(checkinTotal)} checked in of {fmt(rsvpTotal)} RSVPs
            </Text>
          </View>
            ...
            <View key={g.id} style={tab.sessionRow}>
              <View
                style={[
                  ...
                  {
                    backgroundColor:
                      g.status === "ACTIVE" ? semantic.success : neutral[300],
                  },
                ]}
              />
              <Text style={tab.sessionTitle} numberOfLines={1}>
                {g.title}
              </Text>
              <Text style={tab.sessionPlayers}>
              ...
              >
                <Text
                  style={[
                    ...
                    {
                      color:
                        g.status === "ACTIVE" ? semantic.success : neutral[400],
                    },
                  ]}
                >
                  {g.status}
...
function RevenueTab({ eventId }: { eventId: string }) {
  const { data, isLoading, isError, refetch } =
    useGetEventRevenueAnalyticsQuery(eventId);
  ...
  if (!totalRevenue && !completedPurchases && !byStatus.length)
    return (
      <Empty
        icon="cash-outline"
        message="No revenue data yet. Appears once tickets are purchased."
      />
    );
          ...
          <KPICard
            ...
            value={
              completedPurchases > 0
                ? fmtMoney(Math.round(totalRevenue / completedPurchases))
                : "—"
            }
...
function SocialTab({ eventId }: { eventId: string }) {
  const { data, isLoading, isError, refetch } =
    useGetEventSocialAnalyticsQuery(eventId);
  ...
  if (!pcTotal && !totalLikes && !totalShares && !totalComments)
    return (
      <Empty
        ...
        message="No social activity yet. Interactions appear as attendees engage."
      />
    );
  ...
  return (
    <View style={tab.wrap}>
      {/* Combined KPIs */}
      <KPIGrid>
        <View style={{ flexBasis: "31%", flexShrink: 0 }}>
          <KPICard
            label="Likes"
...
function GamesTab({ eventId }: { eventId: string }) {
  const { data, isLoading, isError, refetch } =
    useGetEventGameAnalyticsQuery(eventId);
  ...
  if (!totalSessions)
    return (
      <Empty
        ...
        message="No game sessions have been run for this event yet."
      />
    );

  const rankedWinners = [...winners].sort(
    (a, b) => (a.reward?.rank ?? 0) - (b.reward?.rank ?? 0)
  );
  const medals = ["🥇", "🥈", "🥉"];
          ...
          {sessions.map((s: any) => (
            <View key={s.id} style={tab.sessionRow}>
              <View
                style={[
                  tab.statusDot,
                  { backgroundColor: statusColor[s.status] ?? neutral[300] },
                ]}
              />
              <Text style={tab.sessionTitle} numberOfLines={1}>
                {s.title}
              ...
              <View
                style={[
                  ...
                  {
                    backgroundColor: `${
                      statusColor[s.status] ?? neutral[300]
                    }18`,
                  },
                ]}
              >
                <Text
                  style={[
                    tab.statusPillText,
                    { color: statusColor[s.status] ?? neutral[400] },
                  ]}
                >
                  {s.status}
                </Text>
          ...
          {rankedWinners.map((w: any, i: number) => {
            ...
            return (
              <View key={w.rewardId ?? i} style={tab.winnerRow}>
                ...
                <View style={{ flex: 1 }}>
                  <Text style={tab.winnerName} numberOfLines={1}>
                    {name}
                  </Text>
                ...
                <View style={{ alignItems: "flex-end" }}>
                  <Text style={[tab.rewardVal, { color: BRAND }]}>
                    {fmtReward(w.reward)}
                  </Text>
                  <Text
                    style={[
                      tab.rewardStatus,
                      { color: statusColor2[w.status] ?? neutral[400] },
                    ]}
                  >
...
function PostcardsTab({ eventId }: { eventId: string }) {
  ...
  if (!total)
    return (
      <Empty
        icon="images-outline"
        message="No postcards created for this event yet."
      />
    );
  ...
  return (
    <View style={tab.wrap}>
      ...
      <KPIGrid>
        <View style={{ flexBasis: HALF, flexShrink: 0 }}>
          <KPICard
            label="Total Postcards"
            value={fmt(total)}
            iconName="images-outline"
            color="#0891b2"
          />
        </View>
        {Object.entries(byVis)
          .slice(0, 1)
          .map(([k, v]) => (
        ...
        <SectionCard title="Vibe Tag Engagement">
          {vibeTags.map((vt: any) => (
            <View key={vt.id} style={tab.vtRow}>
              ...
              <Text style={tab.vtName} numberOfLines={1}>
                {vt.name}
              </Text>
          ...
          {topPostcards.slice(0, 10).map((p: any, i: number) => (
            <View key={p.id} style={tab.topPostRow}>
              ...
              <View style={{ flex: 1 }}>
                ...
                {p.caption && (
                  <Text style={tab.topPostCaption} numberOfLines={1}>
                    {p.caption}
                  </Text>
                )}
              </View>
...
export default function AnalyticsScreen() {
  ...
  const renderTab = () => {
    switch (activeTab) {
      case "summary":
        return <SummaryTab eventId={eventId} />;
      case "revenue":
        return <RevenueTab eventId={eventId} />;
      case "social":
        return <SocialTab eventId={eventId} />;
      case "games":
        return <GamesTab eventId={eventId} />;
      case "postcards":
        return <PostcardsTab eventId={eventId} />;
    }
  };
