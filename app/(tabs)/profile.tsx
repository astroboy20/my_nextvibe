...
function PostcardGrid({ items }: { items: PostcardItem[] }) {
  ...
  const renderCard = (item: PostcardItem, idx: number) => {
    ...
    return (
      <TouchableOpacity key={item.id} style={[pc.card, { height: h }]} activeOpacity={0.85}>
  ...
  return (
    <View style={pc.grid}>
      <View style={pc.col}>{left.map((item, i)  => renderCard(item, i * 2))}</View>
      <View style={pc.col}>{right.map((item, i) => renderCard(item, i * 2 + 1))}</View>
    </View>
  );
}
...
export default function ProfileScreen() {
  ...
  // Header is loading until we have activity stats for the first time
  const headerLoading = activityLoading && !activityData;

  const isTabLoading =
    (activeTab === 'events'    && eventsLoading    && !eventsData)    ||
    (activeTab === 'postcards' && postcardsLoading && !postcardsData) ||
    (activeTab === 'tickets'   && ticketsLoading   && !ticketsData);
        ...
        {headerLoading ? (
          ...
          <View style={styles.header}>
            <View style={styles.avatarWrap}>
              <Avatar uri={user?.avatarUrl} name={user?.displayName ?? 'U'} size={88} />
            ...
            <View style={styles.statsRow}>
              ...
              <View style={styles.statDivider} />
              <StatItem value={ticketsData?.meta?.total ?? tickets.length} label="Tickets" />
            </View>

            {/* Actions */}
            <View style={styles.actionsRow}>
              <TouchableOpacity style={styles.editBtn} onPress={() => router.push('/edit-profile')} activeOpacity={0.8}>
          ...
          {TABS.map((tab) => {
            ...
            return (
              <TouchableOpacity
                ...
                activeOpacity={0.7}
              >
                <Ionicons name={tab.icon} size={16} color={active ? brand.primary : neutral[400]} />
...
const styles = StyleSheet.create({
  ...
  dashCard: {
    ...
    backgroundColor: `${brand.primary}08`, gap: 12,
  },
  dashIcon: {
  ...
  empty:      { fontFamily: fontFamily.regular, fontSize: fontSize.sm, color: neutral[400], textAlign: 'center' },
});
...
const pc = StyleSheet.create({
  ...
  imgArea:     { borderRadius: 12, backgroundColor: neutral[100], overflow: 'hidden', alignItems: 'center', justifyContent: 'center' },
